...
def add_wish(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if wish_item:
                messages.info(
                    request, f"{wish_item.product} is already on your wishlist"
                )
        except WishList.DoesNotExist:
            WishList.objects.create(user_id=request.user, product_id=product)
            messages.success(
                request, f"{product.name} poster has been added to your wishlist"
            )
...
def delete_wish(request):
    ...
    if request.method == "POST":
        ...
        wish_item.delete()
        messages.success(
            request, f"{wish_item.product_id.name} has been deleted from your wishlist"
        )
